...
server.post('/create-user', (req, res, next) => {
  usersModel.create(req.params.username,
                    req.params.password,
                    req.params.provider,
...
server.post('/update-user/:username', (req, res, next) => {
  usersModel.update(req.params.username,
                    req.params.password,
                    req.params.provider,
                    req.params.familyName,
                    req.params.givenName,
...
server.post('/find-or-create', (req, res, next) => {
  usersModel.findOrCreate({ id: req.params.username,
...
server.get('/find/:username', (req, res, next) => {
  ...
  .then(user => {
    if (!user) res.send(404, new Error('Did not find ' + req.params.username));
    else res.send(user);
