...
exports.userPasswordCheck = function(username, password) {
  ...
  .then(user => {
    if (!user) {
      return { check: false, username: username, message: 'Either username or password is incorrect.' };
...
exports.findOrCreate = function(profile) {
  return exports.find(profile.id).then(user => {
    if (user) return user;
    return exports.create(profile.id, profile.password, profile.provider, profile.familyName,
